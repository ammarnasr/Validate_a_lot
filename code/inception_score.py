...
tf.app.flags.DEFINE_string('checkpoint_dir', './inception_finetuned_models/birds_valid299/model.ckpt', """Path where to read model checkpoints.""")
        ...
        with tf.Session(config=config) as sess:
            with tf.device("/gpu:%d" % FLAGS.gpu):
                ...
                for i in imagePathList :
                    images = load_data(i)
                    incept_mean, incept_std = get_inception_score(sess, images, pred_op)
                    mean_scores.append(incept_mean)
                    std_scores.append(incept_std)
                    
                with open('/content/mean_scores.pkl', 'wb') as f:
                    pickle.dump(mean_scores, f)
                    
                with open('/content/std_scores.pkl', 'wb') as f:
